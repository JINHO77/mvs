    ...
    const run = async () => {
      ...
      if (error) {
        if (!isMissingAccountStatusError(error)) {
          console.error("Teacher layout profile load failed:", toPrettyErrorString(error), error);
        ...
        if (fallbackError) {
          console.error("Teacher layout role fallback failed:", toPrettyErrorString(fallbackError), fallbackError);
          router.replace("/login");
          return;
        }
