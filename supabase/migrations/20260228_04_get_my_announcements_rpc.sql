-- Stable inbox RPC for announcements page.
-- Returns list + summary counts in one call to reduce RLS-sensitive fan-out queries.
...
as $$
  ...
  enriched as (
    ...
    left join lateral (
      select
        string_agg(label, ', ' order by label) as target_summary
      from (
        select distinct
          case
  ...
  paged as (
    ...
    from filtered
    order by
      case when p_sort = 'unread_first' then (case when is_read then 1 else 0 end) else 0 end asc,
      created_at desc
...
grant execute on function public.get_my_announcements(text, text, boolean, text, integer, integer) to authenticated;
