-- Cut off visible announcements for newly joined students and align mailbox RPC.
-- Note: We intentionally use only profiles.created_at as cutoff because it is guaranteed
-- in current schema. approved_at/account_activated_at can be added later when standardized.
...
as $$
  ...
  where public.can_current_user_read_announcement(a.id)
    ...
    and (
      not exists (select 1 from me where role = 'student')
      or exists (
        select 1
        from me
        where role = 'student'
          and (joined_at is null or a.created_at >= joined_at)
      )
    )
